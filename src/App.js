...
const EntertainmentApp = () => {
  ...
  // Auto-detect region with caching
  useEffect(() => {
    const cachedRegion = localStorage.getItem('userRegion');
    if (cachedRegion) {
      setAutoRegion(cachedRegion);
      ...
      const detectRegion = async () => {
        try {
          ...
          setAutoRegion(country);
          setRegion(country);
          localStorage.setItem('userRegion', country);
        } catch (error) {
  ...
  const createFloatingEmoji = (emoji, x, y) => {
    ...
    setFloatingEmojis(prev => [...prev, newEmoji].slice(-5)); // Limit to 5 emojis
    setTimeout(() => {
      setFloatingEmojis(prev => prev.filter(e => e.id !== id));
    }, 1500); // Reduced duration for performance
  };
  ...
  const handleRecommendation = async () => {
    ...
    const cacheKey = `recs_${userThoughts}_${mood}_${region || autoRegion}_${language}_${genre}_${year}`;
    ...
    try {
      ...
      localStorage.setItem(cacheKey, JSON.stringify(fallbackRecommendations));
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-indigo-900 via-purple-900 to-pink-900 relative overflow-x-hidden touch-manipulation">
      {/* Animated Background */}
      <div className="absolute inset-0 overflow-hidden">
        <div className="absolute -top-1/2 -left-1/2 w-[150%] h-[150%] sm:w-full sm:h-full bg-gradient-to-r from-blue-400/20 to-purple-400/20 rounded-full animate-pulse"></div>
        <div className="absolute -bottom-1/2 -right-1/2 w-[150%] h-[150%] sm:w-full sm:h-full bg-gradient-to-l from-pink-400/20 to-orange-400/20 rounded-full animate-pulse delay-1000"></div>
      </div>
      ...
      {floatingEmojis.map((emoji) => (
        <div
          key={emoji.id}
          className="fixed pointer-events-none z-50 text-3xl sm:text-4xl animate-ping"
          style={{
            left: emoji.x,
            top: emoji.y,
      ...
      <div className="bg-white/10 backdrop-blur-md border-b border-white/20 sticky top-0 z-40">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 py-4">
          <div className="flex items-center space-x-3">
            <div className="bg-gradient-to-r from-purple-500 to-pink-500 p-2 sm:p-3 rounded-2xl shadow-2xl">
              <Sparkles className="w-6 h-6 sm:w-8 sm:h-8 text-white animate-spin" style={{animationDuration: '3s'}} />
            </div>
            <div>
              <h1 className="text-2xl sm:text-3xl font-black bg-gradient-to-r from-white to-purple-200 bg-clip-text text-transparent">
                MoodFlix
              </h1>
              <p className="text-xs sm:text-sm text-purple-200 font-medium">✨Entertainment Recommendations</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 py-6 sm:py-8 relative z-10">
        {/* Selection Panel */}
        <div className="bg-white/10 backdrop-blur-xl rounded-3xl shadow-2xl border border-white/20 p-6 sm:p-8 mb-6 sm:mb-8 hover:shadow-purple-500/25 transition-all duration-500">
          <h2 className="text-2xl sm:text-3xl font-bold text-white mb-6 sm:mb-8 flex items-center">
            <Heart className="w-6 h-6 sm:w-8 sm:h-8 mr-3 sm:mr-4 text-pink-400 animate-bounce" />
            Tell us how you're feeling today!
          </h2>

          {/* User Thoughts Input */}
          <div className="mb-6 sm:mb-8">
            <label className="block text-base sm:text-lg font-bold text-white mb-3 sm:mb-4 flex items-center">
            ...
            <div className="relative">
              <textarea
                ...
                className="w-full px-4 sm:px-6 py-3 sm:py-4 rounded-2xl border-2 border-white/30 focus:ring-2 focus:ring-purple-400 focus:border-purple-400 bg-white/10 backdrop-blur-sm text-white placeholder-white/70 resize-none h-28 sm:h-32 transition-all duration-300 hover:bg-white/15 text-sm sm:text-base"
                rows="4"
              />
              <div className="absolute bottom-3 sm:bottom-4 right-3 sm:right-4 text-white/50 text-xs sm:text-sm">
                {userThoughts.length}/500
              </div>
            </div>
            <p className="text-purple-200 text-xs sm:text-sm mt-2 sm:mt-3 flex items-center">
          ...
          <div className="mb-6 sm:mb-8">
            <label className="block text-base sm:text-lg font-bold text-white mb-3 sm:mb-4">Quick Mood Selection (opt.)✨</label>
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3 sm:gap-4">
              {moods.map((moodOption) => (
                <button
                  key={moodOption.value}
                  onClick={(e) => handleMoodClick(moodOption.value, e)}
                  className={`p-4 sm:p-6 rounded-2xl border-2 transition-all duration-300 hover:scale-105 hover:-translate-y-1 group touch-manipulation ${
                    mood === moodOption.value
                      ? `bg-gradient-to-r ${moodOption.color} border-white shadow-2xl ${moodOption.bgGlow} text-white scale-105`
                      : 'bg-white/10 border-white/30 text-white hover:bg-white/20 hover:border-white/50'
                  }`}
                >
                  <div className="text-3xl sm:text-4xl mb-2 sm:mb-3 group-hover:animate-bounce">{moodOption.emoji}</div>
                  <div className="text-xs sm:text-sm font-bold">{moodOption.label}</div>
                </button>
              ))}
            </div>
          </div>
          ...
          <div className="mb-4 sm:mb-6">
            <label className="block text-base sm:text-lg font-bold text-white mb-2 sm:mb-3 flex items-center">
            ...
            <div className="flex flex-col sm:flex-row sm:space-x-4">
              <input
                ...
                className="flex-1 px-4 sm:px-6 py-3 sm:py-4 rounded-xl border border-white/30 focus:ring-2 focus:ring-purple-400 focus:border-transparent bg-white/10 backdrop-blur-sm text-white placeholder-white/70 text-sm sm:text-base"
              />
              {regionLoading && (
                <div className="flex items-center px-4 sm:px-6 py-3 sm:py-4 bg-blue-500/20 rounded-xl backdrop-blur-sm mt-2 sm:mt-0">
                  <div className="animate-spin rounded-full h-4 w-4 sm:h-5 sm:w-5 border-b-2 border-blue-400"></div>
                  <span className="ml-2 sm:ml-3 text-xs sm:text-sm text-blue-200">Detecting...</span>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 sm:gap-6 mb-6 sm:mb-8">
            <div>
              <label className="block text-base sm:text-lg font-bold text-white mb-2 sm:mb-3 flex items-center">
                <Globe className="w-4 h-4 sm:w-5 sm:h-5 mr-1 sm:mr-2 text-green-400" />
                Language (opt.)🗣️
              </label>
              <input
                type="text"
                ...
                className="w-full px-4 sm:px-6 py-3 sm:py-4 rounded-xl border border-white/30 focus:ring-2 focus:ring-purple-400 focus:border-transparent bg-white/10 backdrop-blur-sm text-white placeholder-white/70 text-sm sm:text-base"
              />
            </div>
            <div>
              <label className="block text-base sm:text-lg font-bold text-white mb-2 sm:mb-3 flex items-center">
                <Film className="w-4 h-4 sm:w-5 sm:h-5 mr-1 sm:mr-2 text-orange-400" />
                Genre (opt.)🎭
              </label>
              <input
                type="text"
            ...
            <div>
              <label className="block text-base sm:text-lg font-bold text-white mb-2 sm:mb-3 flex items-center">
                <Calendar className="w-4 h-4 sm:w-5 sm:h-5 mr-1 sm:mr-2 text-purple-400" />
                Year (opt.)📅
              </label>
              <input
                type="number"
          ...
          <button
            ...
            className="w-full bg-gradient-to-r from-purple-500 via-pink-500 to-purple-500 text-white py-4 sm:py-6 px-6 sm:px-8 rounded-2xl font-bold text-lg sm:text-xl hover:from-purple-600 hover:via-pink-600 hover:to-purple-600 transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center space-x-2 sm:space-x-3 shadow-2xl hover:shadow-purple-500/50 hover:scale-105 bg-size-200 animate-gradient touch-manipulation"
          >
            {loading ? (
              <>
        ...
        {showRecommendations && (
          <div className="flex justify-center mb-6 sm:mb-8 animate-bounce">
            <button
              ...
              className="bg-white/20 backdrop-blur-sm p-3 sm:p-4 rounded-full text-white hover:bg-white/30 transition-all duration-300 touch-manipulation"
            >
        ...
        {showRecommendations && recommendations.length > 0 && (
          <div 
            ...
            className="bg-white/10 backdrop-blur-xl rounded-3xl shadow-2xl border border-white/20 p-6 sm:p-8 animate-slideIn"
          >
            <h3 className="text-2xl sm:text-3xl font-bold text-white mb-6 sm:mb-8 flex items-center">
              <Star className="w-6 h-6 sm:w-8 sm:h-8 mr-3 sm:mr-4 text-yellow-400 animate-pulse" />
            ...
            <div className="mb-6 sm:mb-8">
              <div className="flex flex-wrap gap-3 sm:gap-4 justify-center">
                <button
                  ...
                  className={`px-4 sm:px-6 py-2 sm:py-3 rounded-full font-bold transition-all duration-300 flex items-center space-x-1 sm:space-x-2 touch-manipulation ${
                    activeFilter === 'all'
              ...
              {recommendations.map((item, index) => (
                ...
                >
                  <div className="flex flex-col sm:flex-row">
                    <div className="w-full sm:w-32 md:w-40 h-48 sm:h-60 flex-shrink-0 relative overflow-hidden">
                      <img
                        src={item.poster}
                        alt={item.title}
                        loading="lazy"
                    ...
                    <div className="p-4 sm:p-6 flex-1 text-white">
                      <div className="flex items-start justify-between mb-3 sm:mb-4">
                        <div>
                          <h4 className="text-lg sm:text-xl font-bold mb-2 group-hover:text-purple-200 transition-colors">
                            {item.title}
                          </h4>
                          <div className={`inline-flex items-center px-2 sm:px-3 py-1 rounded-full text-xs font-bold bg-gradient-to-r ${getTypeColor(item.type)} text-white shadow-lg`}>
                            {getTypeIcon(item.type)}
                            <span className="ml-1">{item.type}</span>
                          </div>
                        </div>
                        <div className="text-right">
                          <div className="flex items-center bg-yellow-500/20 px-2 sm:px-3 py-1 rounded-full mb-1 sm:mb-2">
                            <Star className="w-3 h-3 sm:w-4 sm:h-4 text-yellow-400 mr-1" />
                            <span className="text-yellow-200 font-bold text-xs sm:text-sm">{item.rating}</span>
                          </div>
                          <div className="text-purple-200 text-xs sm:text-sm font-medium">{item.year}</div>
                        </div>
                      </div>
                      ...
                      <div className="mb-3 sm:mb-4">
                        <div className="text-cyan-200 text-xs sm:text-sm font-medium mb-1 sm:mb-2">💡 Why this matches:</div>
                        <p className="text-purple-200 text-xs sm:text-sm italic">{item.reason}</p>
                      </div>

                      <div className="mb-3 sm:mb-4">
                        <div className="text-xs sm:text-sm text-gray-300 mb-1 sm:mb-2">Available on:</div>
                        <div className="flex flex-wrap gap-2">
                          {item.platforms && item.platforms.map((platform, idx) => (
                            <span
                              key={idx}
                              className={`px-2 sm:px-3 py-1 text-xs font-bold rounded-full ${getPlatformColor(platform)}`}
                            >
                              {platform}
                            </span>
                          ))}
                      ...
                      <div className="flex items-center justify-between text-xs sm:text-sm">
                        ...
                        <button className="bg-gradient-to-r from-purple-500 to-pink-500 hover:from-purple-600 hover:to-pink-600 text-white px-3 sm:px-4 py-1 sm:py-2 rounded-lg font-bold transition-all duration-300 hover:scale-105 flex items-center space-x-1 sm:space-x-2 touch-manipulation">
            ...
            {recommendations.length === 0 && (
              <div className="text-center py-10 sm:py-12">
                ...
                <p className="text-white text-lg sm:text-xl">No recommendations found for this filter.</p>
                <p className="text-purple-200 text-sm sm:text-base">Try selecting a different category!</p>
              </div>
            )}
          </div>
        )}
        ...
        {/* Footer */}
        <div className="text-center mt-12 sm:mt-16 text-purple-200">
          <div className="text-3xl sm:text-4xl mb-3 sm:mb-4">✨</div>
          <p className="text-base sm:text-lg font-medium">Made with 💜 for entertainment lovers</p>
          <p className="text-xs sm:text-sm mt-1 sm:mt-2 opacity-75">Powered by G1 • Personalized for you</p>
        </div>
      </div>
